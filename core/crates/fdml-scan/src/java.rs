...
impl JavaScanner {
    pub fn parse_file(source: &str, file_path: &str) -> Result<FileAnalysis> {
        ...
        parser
            ...
            .map_err(|e| anyhow::anyhow!(format!("Failed to set Java language: {}", e)))?;
    ...
    /// Collect `method_invocation` call sites, tracking the enclosing method.
            ...
            if child.kind() == "method_invocation" {
                if let Some(name) = child.child_by_field_name("name") {
                    out.push(CallRef {
                        ...
                        callee: Self::node_text(&name, source).to_string(),
