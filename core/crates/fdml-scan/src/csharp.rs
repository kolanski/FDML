...
impl CSharpScanner {
    pub fn parse_file(source: &str, file_path: &str) -> Result<FileAnalysis> {
        ...
        parser
            ...
            .map_err(|e| anyhow::anyhow!(format!("Failed to set C# language: {}", e)))?;
        ...
        Ok(FileAnalysis {
            ...
            calls: {
                let mut c = Vec::new();
                Self::collect_calls(&root, source, "", &mut c);
                c
            },
        })
    }
    ...
    /// Collect `invocation_expression` call sites, tracking the enclosing method.
    fn collect_calls(node: &Node, source: &str, enclosing: &str, out: &mut Vec<CallRef>) {
        ...
        for child in node.children(&mut cursor) {
            ...
            let next = match child.kind() {
                "method_declaration" | "constructor_declaration" | "local_function_statement" => child
    ...
    /// `Foo()` -> "Foo", `obj.Method()` / `this.Method()` -> "Method" (member name).
