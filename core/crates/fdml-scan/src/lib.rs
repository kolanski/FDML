//! Pass 2: tree-sitter code scanner. Ported faithfully from FDML's `src/scanner/mod.rs`.
//!
//! Public API is `run(root, exclude) -> ScanResult`. The per-file `parse_file` functions
//! (one per language module) stay pure so parsing can be parallelized later in one line —
//! today it is deliberately SEQUENTIAL (no rayon).
//!
//! Determinism: collected source files are sorted by relative path before parsing, so
//! `files[]` order is reproducible regardless of filesystem `read_dir` order. The old
//! `scan_timestamp` (`Utc::now()`) metadata field is gone for the same reason.
...
pub fn run(root: &Path, exclude_patterns: &[String]) -> Result<ScanResult> {
    if !root.exists() {
        return Err(anyhow::anyhow!("Path does not exist: {}", root.display()));
    }

    let base_path = std::fs::canonicalize(root).unwrap_or_else(|_| root.to_path_buf());
    ...
    // Determinism: sort by relative path so files[] order is independent of read_dir order.
    source_files.sort_by(|a, b| make_relative(&base_path, &a.0).cmp(&make_relative(&base_path, &b.0)));
    ...
    for (abs_path, language) in &source_files {
        let source = std::fs::read_to_string(abs_path).map_err(|e| {
            anyhow::anyhow!("Failed to read {}: {}", abs_path, e)
        })?;
...
fn collect_files(
    ...
    for entry in entries {
        let entry = entry.map_err(|e| {
            anyhow::anyhow!("Failed to read entry: {}", e)
        })?;
        let path = entry.path();
