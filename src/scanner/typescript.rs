...
impl TypeScriptScanner {
    pub fn parse_file(source: &str, file_path: &str) -> Result<FileAnalysis> {
        ...
        parser
            ...
            .map_err(|e| crate::error::FdmlError::simple_parser_error(format!("Failed to set TypeScript language: {}", e)))?;

        let tree = parser
            .parse(source, None)
            .ok_or_else(|| crate::error::FdmlError::simple_parser_error("Failed to parse TypeScript file"))?;
